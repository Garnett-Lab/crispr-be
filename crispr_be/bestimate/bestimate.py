...
# This is a modified version of an older version of Bestimate, optimized for performances
# The up to date Bestimate code is available on https://github.com/Garnett-Lab/BEstimate
...
# changed to support more requests
from requests.adapters import HTTPAdapter, Retry
from tqdm import tqdm

s = requests.Session()
retries = Retry(total=500, backoff_factor=0.1, status_forcelist=[110])
...
def take_input():
    ...
    parser.add_argument(
        ...
        dest="GENE2ENSG",
        required=True,
        help="JSON dict linking gene names to ENSG ensembl id",
    )
...
class Ensembl:
    ...
    def __init__(
        ...
        with open(gene2ensg_path, "r") as f:
            self.gene2ensg = json.load(f)
    ...
    def extract_sequence(self, gene_id, mutations=None):
        ...
        seq_request = s.get(seq_ensembl, headers={"Content-Type": "text/x-fasta"})
        seq_flan_request = s.get(
        ...
        if self.strand == -1:
            self.flan_gene_range = [self.flan_gene_range[1], self.flan_gene_range[0]]

        # Preparation of the Ensembl sequence for analysis

        nucleotide_dict = {"A": "T", "T": "A", "G": "C", "C": "G"}

        # print ('ensembl', mutations)
        ...
        if mutations is None:
            ...
            for mutation in mutations:
                # 	print (mutation.split(":")[0] , self.chromosome)
                if mutation.split(":")[0] == self.chromosome:
                    # 		print (mutation.split(":")[1].split(".")[0])
    ...
    def extract_info(self, chromosome, loc_start, loc_end, transcript=None):
        ...
        ensembl = (
            ...
            "feature=cds" % (chromosome, int(loc_start), int(loc_end))
        )
        ...
        if request.status_code != 200:
            ...
            for output in request.json():
                ...
                if transcript is None:
                    if (
                        ...
                        and (
                            output["source"] == "ensembl_havana"
                            or output["source"] == "havana"
                        )
                    ):
                            ...
                            self.info_dict[output["id"]] = [
                                {
                                    "start": output["start"],
                                    "end": output["end"],
                                    "biotype": output["biotype"],
                                    "canonical": output["is_canonical"],
                    ...
                    if (
                        ...
                        transcript_info = (
                            "/lookup/id/%s?expand=1;mane=1" % output["transcript_id"]
                        )
                        transcript_request = s.get(
                        ...
                        if output["transcript_id"] not in self.info_dict.keys():
                            self.info_dict[output["transcript_id"]] = [
                                {
                                    ...
                                    "biotype": transcript_output["biotype"],
                                    "canonical": transcript_output["is_canonical"],
                                    "source": output["source"],
                                }
                            ]
                            ...
                            if {
                                "start": transcript_request["start"],
                                "end": transcript_request["end"],
                                "biotype": transcript_request["biotype"],
                                "canonical": transcript_request["is_canonical"],
                                "source": output["source"],
                            } not in old_val:
                                old_val.append(
                                    {
            ...
            for output in request.json():
                if (
                    ...
                    and self.info_dict != {}
                    and output["Parent"] in self.info_dict.keys()
                    and (
                        output["source"] == "ensembl_havana"
                        or output["source"] == "havana"
                    )
                ...
                for transcript_dict in transcript_dict_list:
                    ...
                    for loc in range_locations:
                        if transcript_dict["start"] <= loc <= transcript_dict["end"]:
                            # if transcript_dict['start'] < end or transcript_dict['end'] > start:
                            in_transcript = True
                    if in_transcript:
...
h_sapiens_interfaces_path = os.path.join(
    os.path.dirname(__file__), "data/H_sapiens_interfaces.txt"
)
yulab = pandas.read_table(h_sapiens_interfaces_path)
...
def extract_grna_sites(
    ...
    for direction, crispr in crisprs_dict.items():
        ...
        for cr in tqdm(crispr):
            ...
            # if crispr_seq == 'CCCAGAGGAGGCTCAAAGGGTGA':
            # 	import pdb;pdb.set_trace()
    ...
    crisprs_df = pandas.concat([crisprs_df] + crispr_dfs)
...
def find_editable_nucleotide(
    crispr_df, searched_nucleotide, activity_window, ensembl_object: Ensembl, mutations
):
    # print ('aaaaaaaa', mutations)
    """
    ...
    start_location = crispr_df["Location"].apply(
        lambda x: x.split(":")[1].split("-")[0]
    )
    end_location = crispr_df["Location"].apply(lambda x: x.split(":")[1].split("-")[1])

    for ind, row in tqdm(crispr_df.iterrows(), total=crispr_df.shape[0]):

        range_seq = list(range(0, len(row["gRNA_Target_Sequence"])))
        # Check only with the sequence having PAM since it only has the searched nucleotide!
        possible_inds = list(range(activity_window[0], activity_window[1]))
        searched_ind = []
        for nuc_ind in range_seq:
            if (
                nuc_ind in possible_inds
                and row["gRNA_Target_Sequence"][nuc_ind] == searched_nucleotide
            ):
                searched_ind.append(nuc_ind)

        # searched_ind = [nuc_ind
        ...
        if searched_ind is not []:
            ...
            if row["Direction"] == "left":
                for nuc_ind in searched_ind:
                    try:
                        location = int(end_location[ind]) - nuc_ind
                    except:
                        import pdb

                        pdb.set_trace()
                    if actual_seq_range[0] <= location < actual_seq_range[1]:
                        actual_inds.append(location)

            elif row["Direction"] == "right":
                for nuc_ind in searched_ind:
                    location = int(start_location[ind]) + nuc_ind
                    if actual_seq_range[0] <= location < actual_seq_range[1]:
                        actual_inds.append(location)
            ...
            for actual_ind in actual_inds:
                ...
                # if edit_in_cds and not edit_in_exon:
                # 	import pdb;pdb.set_trace()
    ...
    edit_df = pandas.concat([edit_df] + edit_dfs)
    print("Edit df done")
    # edit_df.to_csv(edit_df_path, index=False)
    # import pdb;pdb.set_trace()

    # else:
    # 	edit_df = pandas.read_csv(edit_df_path)
    ...
    # for guide, g_df in tqdm(edit_df.groupby(["gRNA_Target_Sequence"])):
    # 	# import pdb;pdb.set_trace()
    # 	# unique_edits_per_guide = len(set(list(g_df["Edit_Location"])))
    # 	unique_edits_per_guide = len(g_df["Edit_Location"].unique())
    # 	# edit_df.loc[edit_df.gRNA_Target_Sequence == guide, "# Edits/guide"] = unique_edits_per_guide
    # 	edit_df.loc[g_df.index, "# Edits/guide"] = unique_edits_per_guide

    edit_df["Poly_T"] = edit_df.apply(
        lambda x: (
            True if re.search("T{4,}", x.CRISPR_PAM_Sequence) is not None else False
        ),
        axis=1,
    )
...
def extract_hgvs(
    ...
    if transcript_id is not None:
        ...
        for transcript, transcript_dict in ensembl_object.info_dict.items():
            # print (transcript, transcript_dict)
            for d in transcript_dict:
                # 	print (d)
                if (
                    "havana" in d["source"]
                ):  # == "ensembl_havana" or d["source"] == "havana" :
            ...
            for grna, grna_df in direction_df.groupby(["gRNA_Target_Sequence"]):
                ...
                for edit_loc, grna_edit_df in grna_df.groupby(["Edit_Location"]):
                    ...
                    if True not in grna_edit_df.guide_on_mutation.unique():
                        ...
                        len(list(grna_edit_df.guide_on_mutation.unique())) == 1
                        and list(grna_edit_df.guide_on_mutation.unique())[0] is None
                    ):
                    ...
                    d = {
                        ...
                        "guide_on_mutation": grna_edit_df["guide_on_mutation"].values[
                            0
                        ],
...
def retrieve_vep_info(hgvs_df, ensembl_object, transcript_id=None):
    ...
    if transcript_id is None:
        for transcript, transcript_dict in ensembl_object.info_dict.items():
            for d in transcript_dict:
                if d["source"] == "ensembl_havana" or d["source"] == "havana":
                    transcript_id = transcript
    ...
    all_vep_dfs = list()
    fh = open("all_cals.txt", "w")
    for ind, row in hgvs_df.iterrows():
        # VEP API request
        vep = (
            "/vep/human/hgvs/%s?Blosum62=1;Conservation=1;Mastermind=1;"
        ...
        vep_request = s.get(server + vep, headers={"Content-Type": "application/json"})
        ...
        if vep_request.status_code != 200:
            ...
            na_df = pandas.DataFrame(
                [
                    [
                        ...
                        row.Transcript_ID,
                        row.Exon_ID,
                        row.guide_on_mutation,
                        row.guide_change_mutation,
                        None,
                        None,
                        None,
                        None,
            ...
            (
                vep_initials,
                vep_transcript_results,
                vep_clinical_results,
                vep_most_severe_results,
            ) = (list(), list(), list(), list())
            vep_transcript_interested = [
                "hgvsc",
                "hgvsp",
                "protein_id",
                "transcript_id",
            ...
            for x in vep_request.json():
                ...
                if "most_severe_consequence" in x.keys():
                    vep_most_severe["most_severe_consequence"] = x[
                ...
                for t in x["transcript_consequences"]:
                    ...
                    if (
                        t["strand"] == strand
                        and t["gene_id"] == ensembl_object.gene_id
                ...
                if "colocated_variants" in x.keys():
                    for c in x["colocated_variants"]:
                        vep_clinical_result = {}
                        if c["start"] == c["end"]:
                            vep_clinical_result["clinical_allele"] = (
                                c["allele_string"]
                                if "allele_string" in c.keys()
                                else None
                            )
                            vep_clinical_result["clinical_id"] = (
                                c["id"] if "id" in c.keys() else None
                            )
                ...
                if vep_most_severe_results:
                    ...
                    vep_m_df = pandas.DataFrame(
                        None, index=[0], columns=["most_severe_consequence"]
                    )
                ...
                vep_df = pandas.merge(vep_df2, vep_i_df, on="merging").drop(
                    columns=["merging"]
                )
            ...
            if VEP_df.empty is False:
                ...
                VEP_df["Transcript_ID"] = VEP_df.apply(
                    lambda x: (
                        x.transcript_id
                        if x.transcript_id is not None
                        and pandas.isna(x.transcript_id) is False
                        and type(x.transcript_id) != float
                        else None
                    ),
                    axis=1,
                )
                ...
                VEP_df["cDNA_Change"] = VEP_df.apply(
                    lambda x: (
                        x.hgvsc.split(":")[1].split(".")[1]
                        if x.hgvsc is not None
                        and pandas.isna(x.hgvsc) is False
                        and type(x.hgvsc) != float
                        else None
                    ),
                    axis=1,
                )
                ...
                VEP_df["Protein_Change"] = VEP_df.apply(
                    lambda x: (
                        x.hgvsp.split(":")[1].split(".")[1]
                        if x.hgvsp is not None
                        and pandas.isna(x.hgvsp) is False
                        and type(x.hgvsp) != float
                        else None
                    ),
                    axis=1,
                )
                ...
                VEP_df["Edited_AA"] = VEP_df.apply(
                    lambda x: (
                        extract_hgvsp(hgvsp=x.hgvsp, which="old_aa")
                        if x.amino_acids is not None
                        and pandas.isna(x.amino_acids) is False
                        and type(x.amino_acids) != float
                        else None
                    ),
                    axis=1,
                )
                ...
                VEP_df["New_AA"] = VEP_df.apply(
                    lambda x: (
                        extract_hgvsp(hgvsp=x.hgvsp, which="new_aa")
                        if x.amino_acids is not None
                        and pandas.isna(x.amino_acids) is False
                        and type(x.amino_acids) != float
                        else None
                ...
                VEP_df["Edited_AA_Prop"] = VEP_df.apply(
                    lambda x: (
                        aa_chem[x.Edited_AA]
                        if x.Edited_AA is not None
                        and x.Edited_AA in aa_chem.keys()
                        and len(x.Edited_AA) == 1
                        else (
                            ";".join(
                                [
                                    aa_chem[i]
                                    for i in x.Edited_AA.split(";")
                                    if i in aa_chem.keys()
                                ]
                            )
                            if x.Edited_AA is not None and len(x.Edited_AA) > 1
                            else None
                        )
                    ),
                    axis=1,
                ...
                VEP_df["New_AA_Prop"] = VEP_df.apply(
                    lambda x: (
                        aa_chem[x.New_AA]
                        if x.New_AA is not None
                        and x.New_AA in aa_chem.keys()
                        and len(x.New_AA) == 1
                        else (
                            ";".join(
                                [
                                    aa_chem[i]
                                    for i in x.New_AA.split(";")
                                    if i in aa_chem.keys()
                                ]
                            )
                            if x.New_AA is not None and len(x.New_AA) > 1
                            else None
                        )
                    ),
                    axis=1,
                ...
                VEP_df["Edited_Codon"] = VEP_df.apply(
                    lambda x: (
                        x.codons.split("/")[0]
                        if x.codons is not None
                        and pandas.isna(x.codons) is False
                        and type(x.codons) != float
                        else None
                    ),
                    axis=1,
                )
                ...
                VEP_df["is_Synonymous"] = VEP_df.apply(
                    lambda x: (
                        True
                        if x.Edited_Codon is not None
                        and x.New_Codon is not None
                        and x.Edited_AA is not None
                        and x.New_AA is not None
                        and x.Edited_Codon != x.New_Codon
                        and x.Edited_AA == x.New_AA
                        else (
                            None
                            if x.Edited_Codon is None
                            and x.New_Codon is None
                            or x.Edited_AA is None
                            and x.New_AA is None
                            else False
                        )
                    ),
                    axis=1,
                ...
                VEP_df["proline_addition"] = VEP_df.apply(
                    lambda x: (
                        True
                        if x.is_Synonymous is not None
                        and x.is_Synonymous == False
                        and x.New_AA is not None
                        and "P" in x.New_AA.split(";")
                        else False
                    ),
                    axis=1,
                )
                ...
                VEP_df["is_Stop"] = VEP_df.apply(
                    lambda x: (
                        ...
                        and len(x.New_AA) == 1
                            ...
                            and "*" in x.New_AA
                            and len(x.New_AA) > 1
                            else (None if x.New_AA is None else False)
                        )
                    ),
                    axis=1,
                )
                ...
                VEP_df["swissprot"] = VEP_df.apply(
                    lambda x: (
                        x.swissprot[0].split(".")[0]
                        if x.swissprot is not None
                        else False
                    ),
                    axis=1,
                )
                ...
                VEP_df["is_clinical"] = VEP_df.apply(
                    lambda x: (
                        True
                        if x.clinical_allele is not None
                        and type(x.clinical_allele) != float
                        and pandas.isna(x.clinical_allele) is False
                        else False
                    ),
                    axis=1,
                )
                ...
                VEP_df["consequence_terms"] = VEP_df.apply(
                    lambda x: (
                        ";".join(x.consequence_terms)
                        if type(x.consequence_terms) == list
                        else x.consequence_terms
                    ),
                    axis=1,
                )
...
def annotate_edits(ensembl_object, vep_df):
    ...
    if ensembl_seq_mapping != {}:
        for ind, row in vep_df.iterrows():
            ...
            if (
                row.Protein_ID is not None
                and row.Protein_ID in ensembl_seq_mapping.keys()
                and ensembl_seq_mapping[row.Protein_ID] is not None
                and ensembl_seq_mapping[row.Protein_ID] != []
            ):
                seq_mapping = ensembl_seq_mapping[row.Protein_ID]
                reviewed = list()
                uniprot_tbc = list()
                if (
                    row["swissprot"] is not None
                    and row["swissprot"] in seq_mapping.keys()
                ):
                ...
                if uniprot_tbc:
                    for uniprot in uniprot_tbc:
                        ...
                        if (
                            row["Protein_Position"] is not None
                            and pandas.isna(row["Protein_Position"]) is False
                        ):
                                ...
                                if (
                                    ...
                                    if int(position) in seq_mapping[uniprot].keys():
                                        dom = uniprot_object.find_domain(
                                            seq_mapping[uniprot][int(position)],
                                            row["Edited_AA"],
                                        )
            ...
            df_d = {
                ...
                "CRISPR_PAM_Sequence": [row["CRISPR_PAM_Sequence"]],
                "CRISPR_PAM_Location": [row["CRISPR_PAM_Location"]],
                "gRNA_Target_Sequence": [row["gRNA_Target_Sequence"]],
                "gRNA_Target_Location": [row["gRNA_Target_Location"]],
                ...
                "guide_in_CDS": [row["guide_in_CDS"]],
                "Edit_in_Exon": [row["Edit_in_Exon"]],
                "Edit_in_CDS": [row["Edit_in_CDS"]],
                "guide_on_mutation": [row["guide_on_mutation"]],
                "guide_change_mutation": [row["guide_change_mutation"]],
                "# Edits/guide": [row["# Edits/guide"]],
                "Poly_T": [row["Poly_T"]],
                "VEP_input": [row["VEP_input"]],
                "hgvsp": [row["hgvsp"]],
                "variant_classification": [row["variant_classification"]],
                "cDNA_Change": [row["cDNA_Change"]],
                "Edited_Codon": [row["Edited_Codon"]],
                "New_Codon": [row["New_Codon"]],
                "Protein_Position": [row["Protein_Position"]],
                "Protein_Change": [row["Protein_Change"]],
                "Edited_AA": [row["Edited_AA"]],
                "Edited_AA_Prop": [row["Edited_AA_Prop"]],
                "New_AA": [row["New_AA"]],
                "New_AA_Prop": [row["New_AA_Prop"]],
                "is_Synonymous": [row["is_Synonymous"]],
                "is_Stop": [row["is_Stop"]],
                "proline_addition": [row["proline_addition"]],
                ...
                "consequence_terms": [row["consequence_terms"]],
                "is_clinical": [row["is_clinical"]],
                "clinical_allele": [row["clinical_allele"]],
                "clinical_id": [row["clinical_id"]],
                "clinical_significance": [row["clinical_significance"]],
                "var_synonyms": [row["var_synonyms"]],
                "most_severe_consequence": [row["most_severe_consequence"]],
                "Domain": [domain],
                "PTM": [ptm],
                "SwissProt_VEP": [row["swissprot"]],
                "Uniprot": [uniprot],
            }
...
def summarise_guides(last_df):
    ...
    for guide, guide_df in last_df.groupby(["CRISPR_PAM_Sequence"]):
        summary_df.loc[i, "Hugo_Symbol"] = ";".join(
            [x for x in list(guide_df.Hugo_Symbol.unique()) if x is not None]
        )
        ...
        summary_df.loc[i, "Edit_Position"] = ";".join(
            [str(x) for x in list(guide_df.Edit_Position.unique()) if x is not None]
        )
        summary_df.loc[i, "Direction"] = ";".join(
            [x for x in list(guide_df.Direction.unique()) if x is not None]
        )
        ...
        summary_df.loc[i, "Exon_ID"] = ";".join(
            [x for x in list(guide_df.Exon_ID.unique()) if x is not None]
        )
        summary_df.loc[i, "Protein_ID"] = ";".join(
            [x for x in list(guide_df.Protein_ID.unique()) if x is not None]
        )
        ...
        summary_df.loc[i, "guide_on_mutation"] = (
            True if True in guide_df.guide_on_mutation.unique() else False
        )
        ...
        summary_df.loc[i, "clinical_allele"] = ";".join(
            [
                x
                for x in list(guide_df.clinical_allele.unique())
                if x is not None and type(x) != float
            ]
        )
        ...
        summary_df.loc[i, "Uniprot"] = ";".join(
            [
                x
                for x in list(guide_df.Uniprot.unique())
                if x is not None and type(x) != float
            ]
        )
            ...
            if args["OUTPUT_FILE"] + "_protein_df.csv" not in os.listdir(path):
                ...
                if len(whole_vep_df.index) != 0:
                    uniprot_df = annotate_edits(
                        ensembl_object=ensembl_obj, vep_df=whole_vep_df
                    )
...
------------------------------------------------------
                    ...
                    if summary_df is not None and len(summary_df.index) != 0:
                        ...
                        print(
                            "Summary Data Frame cannot be created because it is empty."
                        )
